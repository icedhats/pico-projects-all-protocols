...
uint8_t M2M_LM75A::read8bitRegister(const uint8_t reg)
{
	...
	i2c_write_blocking(i2c0, _i2cAddress, &reg, 1, true);
	i2c_read_blocking(i2c0, _i2cAddress, &result, 1, false);
	return result;
}
...
bool M2M_LM75A::read16bitRegister(const uint8_t reg, uint16_t& response)
{
	...
	i2c_write_blocking(i2c0, _i2cAddress, &reg, 1, true);
	i2c_read_blocking(i2c0, _i2cAddress, result, 2, false);

	uint16_t temp = result[0] << 8 | result[1];
	response = result[0] << 8 | result[1];
...
bool M2M_LM75A::write16bitRegister(const uint8_t reg, const uint16_t value)
{
	...
	return i2c_write_blocking(i2c0, _i2cAddress, &reg, 2, false);
}
...
bool M2M_LM75A::write8bitRegister(const uint8_t reg, const uint8_t value)
{
	...
	return i2c_write_blocking(i2c0, _i2cAddress, &reg, 1, false);
}
